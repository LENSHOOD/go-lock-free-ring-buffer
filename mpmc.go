...
// mpmc defines a multi-producer multi-consumer ring buffer which different with mpsc.
...
// node.step to head + mpmc.mask, to announce the Poll completed. The reason head + mpmc.mask
// is to tell the next producer move to this node: "I'm available to be Offer", we can simply
// calculate the next producer should hold the tail of head + mpmc.mask (tail moved over the
// ring back to here).
//
